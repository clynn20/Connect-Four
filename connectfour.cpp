...
void empty_board(char** b,int row, int col){
	for(int i=0;i<row;i++){
		for(int j=0;j<col;j++){
			b[i][j]=' ';
		}
...
//two player mood
void two_player(char** b,int row,int col,int* playerturn){
	int input_col;
...
//one player mood
void one_player(char** b,int row,int col,int* playerturn){
...
int main(int argc, char*argv[]){
	...
	empty_board(board,row,col);
	int playerturn = 1;
	switch(playernum){
		case 1: 
			one_player(board,row,col,&playerturn);
			break;
		case 2:
			two_player(board,row,col,&playerturn);
			break;
	}
